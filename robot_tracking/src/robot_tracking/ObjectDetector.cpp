...
namespace robot_tracking
{
    ...
    void ObjectDetector::image_callback(const sensor_msgs::msg::Image::ConstSharedPtr & msg)
    {
        ...
        try {
            ...
            RCLCPP_ERROR(get_logger(), "cv_bridge_exception : %s ", e.what());
            return;
        }

        cv::Mat img_hsv; //rgb image
        cv::cvtColor(cv_ptr->image, img_hsv, cv::COLOR_BGR2HSV);

        cv::Mat1b filtered; //gray image
        ...
        detection_msg.bbox.size_y = bbx.width;
        detection_msg.bbox.center.position.x = cx;
        detection_msg.bbox.center.position.y = cy;
        detection_pub_->publish(detection_msg);


        // drawing the bondary box and the circular in the image to debug
        if (debug_){
